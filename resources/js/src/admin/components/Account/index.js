			...
			this.props.form.validateFieldsAndScroll((err, values) => {
				if (!err) {
					if (modalType === 'roles') {
						...
						row.key_vehicle = values.key_vehicle;
						row.equipment_no = values.equipment_no;
						let baseImage = '';
						...
						{modalType === 'insert' ? (
							<Form className="FormData">
								<div>
									...
									<FormItem
										{...formItemLayout}
										label={<span>Key Vehicle</span>}
										hasFeedback
										style={{ marginBottom: 10 + 'px' }}
									>
										{getFieldDecorator('key_vehicle', {
											rules: []
										})(<Input onChange={this.handleChange} style={{ width: '100%' }} />)}
									</FormItem>
									<FormItem
										{...formItemLayout}
										label={<span>Equipment No</span>}
										hasFeedback
										style={{ marginBottom: 10 + 'px' }}
									>
										{getFieldDecorator('equipment_no', {
											rules: []
										})(<Input onChange={this.handleChange} style={{ width: '100%' }} />)}
									</FormItem>
							...
							<Form className="FormData">
								<div>
									<FormItem {...formItemLayout} label="Full name" hasFeedback>
										{getFieldDecorator('full_name', {
											initialValue: row && row.full_name ? row.full_name : null,
											rules: [
												{
													required: true,
													message: 'Full name is required'
									...
									>
										{getFieldDecorator('key_vehicle', {
											initialValue: row && row.key_vehicle ? row.key_vehicle : ''
										})}
										{getFieldDecorator('key_vehicle', {
											rules: []
										})(<Input onChange={this.handleChange} style={{ width: '100%' }} />)}
									...
									>
										{getFieldDecorator('equipment_no', {
											initialValue: row && row.equipment_no ? row.equipment_no : ''
										})}
										{getFieldDecorator('equipment_no', {
											rules: []
										})(<Input onChange={this.handleChange} style={{ width: '100%' }} />)}
