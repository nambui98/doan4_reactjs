...
import Modal from "../../components/CheckList";
import actions from "../../redux/CheckList/actions";
...
class CheckList extends BaseComponent {
  componentDidMount() {
    ...
    listenSocket(this.socketIO, flags.CHECKLIST, (flag, data) => {
      const newPage = {
        name: this.state.search,
        page: this.state.page,
        limit: dataConfig.pageLimit
      ...
      <DataWrapper>
        <LayoutWrapper style={{ padding: "0px" }}>
          <h1 className="PageHeader">
            {" "}
            <IntlMessages id="sidebar.checklist" />
          </h1>
