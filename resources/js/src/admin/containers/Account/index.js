...
class Accounts extends BaseComponent {
	...
	componentDidMount() {
		...
		listenSocket(this.socketIO, flags.ADMIN, (flag, data) => {
			const newPage = {
				name: this.state.search,
				page: this.state.page,
				limit: dataConfig.pageLimit
				...
				render: (text, row) => {
					return (
						<img
							...
							src={row.avata ? urlConfig.imageUrl + row.avata : ''}
						/>
					);
				}
			},
            ...
            */
			{
				...
				render: (text, row) => {
					if (
						...
						return (
							<ActionWrapper>
								{JSON.parse(localStorage.getItem('role_detail')).find(
									(role) => role.address === '1'
								) ? (
									<Tooltip title="Edit">
										<button onClick={this.handleModal.bind(this, 'edit', row)} className="aSimple">
