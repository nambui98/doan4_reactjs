...
function* loadData({ payload }) {
  try {
      ...
      let vData = yield callAPI("v1/incidentType/getAll", "POST", {
  ...
  try {
    ...
    switch (actionName) {
      case "changeStatus":
        ...
        break;
      case "insert":
        data_output = yield callAPI("v1/incidentType", "POST", {
          name:data.name,
          color:data.color,
          order:data.order,
          status:data.status
        ...
        break;
      case "delete":
        data_output = yield callAPI("v1/incidentType/" + data.id, "DELETE");
        break;
      case "update":
        data_output = yield callAPI("v1/incidentType/" + data.id, "PUT", {
          name:data.name,
          color:data.color,
          order:data.order,
          status:data.status
