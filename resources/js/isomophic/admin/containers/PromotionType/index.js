...
import Modal from '../../components/PromotionType';
import actions from '../../redux/PromotionType/actions';
		...
		columns = [
			{
				...
				render: (text, row) => {
					return <span>{row.type === 1 ? 'Phầm trăm' : 'Số tiền'}</span>;
				}
			},
			{
				...
				render: (text, row) => {
					return <span>{row.type === 1 ? row.numbersale + ' %' : convertNumber(row.numbersale) + ' ₫'}</span>;
				}
			},
			// {
			// 	title: 'Nhân viên',
			...
			<DataWrapper>
				<LayoutWrapper style={{ padding: '0px' }}>
					<h1 className="PageHeader">
						{' '}
						<IntlMessages id="sidebar.promotionType" />
					</h1>
