...
import Modal from "../../components/Jobsite";
import actions from "../../redux/JobSite/actions";
...
class JobSite extends BaseComponent {
  componentDidMount() {
    const page = {
    ...
    listenSocket(this.socketIO, flags.JOB_SITE, (flag, data) => {
      const newPage = {
        name: this.state.search,
        page: this.state.page,
        limit: dataConfig.pageLimit
      ...
      <DataWrapper>
        <LayoutWrapper style={{ padding: "0px" }}>
          <h1 className="PageHeader">
            {" "}
            <IntlMessages id="sidebar.jobSite" />
          </h1>
