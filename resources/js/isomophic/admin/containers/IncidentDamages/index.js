...
import Modal from "../../components/IncidentDamages";
import actions from "../../redux/IncidentDamages/actions";
...
class IncidentDamages extends BaseComponent {
  componentDidMount() {
    ...
    listenSocket(this.socketIO, flags.INCIDENT_DAMAGE, (flag, data) => {
      const newPage = {
        name: this.state.search,
        page: this.state.page,
        limit: dataConfig.pageLimit
      ...
      <DataWrapper>
        <LayoutWrapper style={{ padding: "0px" }}>
          <h1 className="PageHeader">
            {" "}
            <IntlMessages id="sidebar.incidentDamages" />
          </h1>
