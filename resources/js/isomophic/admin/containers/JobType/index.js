...
import Modal from "../../components/JobType";
import actions from "../../redux/JobType/actions";
...
class JobType extends BaseComponent {
  componentDidMount() {
    const page = {
    ...
    listenSocket(this.socketIO, flags.JOB_TYPE, (flag, data) => {
      const newPage = {
        name: this.state.search,
        page: this.state.page,
        limit: dataConfig.pageLimit
      ...
      <DataWrapper>
        <LayoutWrapper style={{ padding: "0px" }}>
          <h1 className="PageHeader">
            {" "}
            <IntlMessages id="sidebar.jobType" />
          </h1>
