...
function* loadData({ payload }) {
  try {
      ...
      let vData = yield callAPI("v1/incidentSeverity/getAll", "POST", {
  ...
  try {
    ...
    switch (actionName) {
      case "changeStatus":
        ...
        break;
      case "insert":
        data_output = yield callAPI("v1/incidentSeverity", "POST", {
          name:data.name,
          code:data.code,
          status:data.status
        });
        break;
      case "delete":
        data_output = yield callAPI("v1/incidentSeverity/" + data.id, "DELETE");
        break;
      case "update":
        data_output = yield callAPI("v1/incidentSeverity/" + data.id, "PUT", {
          name:data.name,
          code:data.code,
          status:data.status
        });
