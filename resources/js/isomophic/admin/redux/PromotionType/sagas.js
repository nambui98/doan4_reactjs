...
function* loadData({ payload }) {
	try {
		let vData = yield callAPI('v1/promotionType/getAll', 'POST', {
	...
	try {
		...
		switch (actionName) {
			case 'changeStatus':
				...
				break;
			case 'insert':
				data_output = yield callAPI('v1/promotionType', 'POST', {
					type: data.type,
					numbersale: data.numbersale
				});
				break;
			case 'delete':
				data_output = yield callAPI('v1/promotionType/' + data.id, 'DELETE');
				break;
			case 'update':
				data_output = yield callAPI('v1/promotionType/' + data.id, 'PUT', {
					type: data.type,
					numbersale: data.numbersale
				});
				break;
