...
function* loadData({ payload }) {
	try {
		let vData = yield callAPI('v1/checklist/getAll', 'POST', {
	...
	try {
		...
		switch (actionName) {
			case 'changeStatus':
				...
				break;
			case 'insert':
				data_output = yield callAPI('v1/checklist', 'POST', {
					name: data.name,
				...
				break;
			case 'delete':
				data_output = yield callAPI('v1/checklist/' + data.id, 'DELETE');
				break;
			case 'update':
				data_output = yield callAPI('v1/checklist/' + data.id, 'PUT', {
					name: data.name,
					description: data.description,
					status: data.status,
					image: data.image
