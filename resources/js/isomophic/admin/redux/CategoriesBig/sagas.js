...
function* loadData({ payload }) {
	try {
		let vData = yield callAPI('v1/categoriesBig/getAll', 'POST', {
	...
	try {
		...
		switch (actionName) {
			case 'changeStatus':
				...
				break;
			case 'insert':
				data_output = yield callAPI('v1/categoriesBig', 'POST', {
					name: data.name,
					status: data.status
				});
				break;
			case 'delete':
				data_output = yield callAPI('v1/categoriesBig/' + data.id, 'DELETE');
				break;
			case 'update':
				data_output = yield callAPI('v1/categoriesBig/' + data.id, 'PUT', {
					name: data.name,
					status: data.status
				});
				break;
