...
function* loadData({ payload }) {
  try {
      ...
      let vData = yield callAPI("v1/jobType/getAll", "POST", {
  ...
  try {
    ...
    switch (actionName) {
      case "changeStatus":
        ...
        break;
      case "insert":
        data_output = yield callAPI("v1/jobType", "POST", {
          name:data.name,
          order:data.order,
          status:data.status
        });
        break;
      case "delete":
        data_output = yield callAPI("v1/jobType/" + data.id, "DELETE");
        break;
      case "update":
        data_output = yield callAPI("v1/jobType/" + data.id, "PUT", {
          name:data.name,
          order:data.order,
          status:data.status
        });
