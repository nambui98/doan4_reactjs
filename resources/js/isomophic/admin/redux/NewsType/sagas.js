...
function* loadData({ payload }) {
    try {
        let vData = yield (callAPI("v1/news_types/getAll", "POST", {
...
function* actionForm({ payload }) {
    ...
    try {
        ...
        switch (actionName) {
        case 'insert':
            data_output = yield callAPI("v1/news_types", "POST", {
                name: data.name,
            });
            break;
        case 'delete':
            data_output = yield callAPI("v1/news_types/" + data.id, "DELETE");
            break;
        case 'update':
            data_output = yield callAPI("v1/news_types/" + data.id, "PUT", {
                name: data.name,
            });
            break;
        default:
