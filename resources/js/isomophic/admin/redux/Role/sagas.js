...
function* loadData({ payload }) {
	try {
		let vData = yield callAPI('v1/role/getAll', 'POST', {
			name: payload.data.name,
			limit: payload.data.limit,
			page: payload.data.page
		});
		// let vData = yield callAPI('v1/role/getAll', 'GET', null);
		// console.log(vData);
...
function* actionFrom({ payload }) {
	...
	try {
		...
		switch (actionName) {
			case 'insert':
				console.log(data);
				data_output = yield callAPI('v1/role', 'POST', {
					rolename: data.name,
				...
				break;
			case 'delete':
				data_output = yield callAPI('v1/role/' + data.id, 'DELETE');
				break;
			case 'update':
				data_output = yield callAPI('v1/role/' + data.id, 'PUT', {
					rolename: data.name,
					address: data.address,
					status: 1
				});
